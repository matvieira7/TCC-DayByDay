document.addEventListener('DOMContentLoaded', function () {
        ...
        eventDrop: function (info) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    edit_start: newStart,
                    edit_end: newEnd,
                    edit_title: info.event.title,
                    edit_color: info.event.backgroundColor,
                    edit_obs: info.event.extendedProps.obs
                },
    ...
    function updateTitle() {
        const titleElement = document.querySelector('.fc-toolbar-title'); // Encontre o título da data
        if (titleElement) {
            const currentDate = calendar.getDate(); // Pegue a data atual do calendário
            const options = { year: 'numeric', month: 'long' }; // Opções para o formato de mês e ano
            const formattedDate = currentDate.toLocaleDateString('pt-BR', options); // Formatar como "Novembro de 2024"

            // Divida a string em partes
            const splitDate = formattedDate.split(' de ');

            // Agora formatamos o mês corretamente, deixando a primeira letra em maiúscula
            const formattedMonth = splitDate[0].charAt(0).toUpperCase() + splitDate[0].slice(1).toLowerCase();

            // Atualize o título
            titleElement.textContent = `${formattedMonth} de ${splitDate[1]}`;
        }
    }
    ...
    if (btnApagarEvento) {
        btnApagarEvento.addEventListener("click", () => {
        ...
        confirmYes.addEventListener("click", async () => {
            ...
            confirmModal.style.display = "none"; // Fecha o modal após a ação
        });

        confirmNo.addEventListener("click", () => {
            confirmModal.style.display = "none"; // Fecha o modal sem realizar a ação
        });
    }
